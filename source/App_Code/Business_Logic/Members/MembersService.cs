...
namespace Business_Logic.Members
{
    ...
    public static class MemberService
    {
        ...
        {
            var dt = Connect.GetData("SELECT * FROM nhsAllowed WHERE nhsID='" + id + "'","nhsAllowed");
        ...
        {
            var dt = Connect.GetData("SELECT * FROM nhsMembers", "nhsMembers");
        ...
        {
            var dt = Connect.GetData("SELECT nhsUserID,nhsFirstName,nhsLastName,nhsType,nhsActive FROM nhsMembers", "nhsMembers");
            return (from DataRow dr in dt.Rows
                select new Member()
                {
                    UserID = int.Parse(dr["nhsUserID"].ToString()),
        ...
        {
            var dt = Connect.GetData("SELECT * FROM nhsMembers,nhsCities WHERE nhsCities.nhsCityID=nhsMembers.nhsCityID AND nhsMembers.nhsActive=Yes", "nhsMembers");
            return dt;
        }
        /// <summary>
        ...
        {
            var dt = Connect.GetData("SELECT * FROM nhsMembers,nhsCities WHERE nhsCities.nhsCityID=nhsMembers.nhsCityID AND nhsMembers.nhsActive=Yes AND nhsMembers.nhsUserID=" + GetCurrent().UserID, "nhsMembers");
            return dt;
        }
        /// <summary>
        ...
        public static bool Login(string email, string pass)
        {
            email = email.Replace("'", "");
            pass = pass.Replace("'", "");
            var dt = Connect.GetData("SELECT * FROM nhsMembers WHERE nhsMail='" + email + "' AND nhsActive=Yes", "nhsMembers");//Excuting a query to check up with the data base
        ...
        public static bool Login(string id, string pass,bool ids)
        {
            id = id.Replace("'", "");
            pass = pass.Replace("'", "");
            var dt = Connect.GetData("SELECT * FROM nhsMembers WHERE nhsID='" + id + "' AND nhsActive=Yes", "nhsMembers");//Excuting a query to check up with the data base
            if (dt.Rows.Count == 0) return false;
            var dbHash = dt.Rows[0]["nhsPass"].ToString();//Getting dbHash - The encrypted password that came from the data base for that user
        ...
        public static void Add(Member m, string pass)
        {
            ...
            foreach (Majors.Major maj in m.Majors)
            {
                ...
                DataTable dt = Connect.GetData("SELECT * FROM nhsMajorsTeacherGrades WHERE nhsMajorID="+maj.Id+" AND nhsGradePart='"+ TeacherGradeService.GetParTeacherGrade(GradesService.Get(m.GradeID).Name).Replace("'", "''")+"'", "nhsMajorsTeacherGrades");
        ...
        public static bool Update(Member m)
        {
            ...
            if (m.UserID == GetCurrent().UserID)
                UpdateCurrent(GetAll().First(x => x.UserID == m.UserID));
            return true;
        }
        /// <summary>
